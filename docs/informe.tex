...
La Fig.~\ref{fig:corr_completa} muestra la matriz de correlación completa sobre las columnas numéricas del dataset, y la Fig.~\ref{fig:corr} su detalle sobre un subconjunto de variables seleccionadas por relevancia predictiva (goles, tiros, cuotas medias y \textit{squad}). Por el momento ambas matrices se presentan de forma preliminar; en \textit{feature engineering} se validarán con más detalle y se decidirá la selección final de variables redundantes. Se confirma \textbf{multicolinealidad}: las cuotas del mismo mercado correlacionan con $|r|>0.9$, por lo que aportan información repetida; esto motiva conservar solo \texttt{AvgH}/\texttt{AvgD}/\texttt{AvgA} como representación compacta del mercado, junto con las banderas \texttt{\_Ausente}. Por otro lado, la \textit{squad} correlaciona poco con las cuotas, lo que la convierte en señal complementaria al mercado.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{eda_corr_completa.png}
\caption{Matriz de correlación sobre las columnas numéricas del dataset.}
\label{fig:corr_completa}
\end{figure}
